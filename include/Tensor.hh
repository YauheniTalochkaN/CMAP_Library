...
namespace CMAP
{
    ...
    class Tensor 
    {
        ...
        Tensor(const std::vector<size_t>& shape, T value) : shape(shape), total_size(1) 
        {
            compute_strides();
        ...
        std::vector<T> get_slice_1d(size_t index) const
        {
            if(this->get_shape().size() != 2)
            {
                throw std::runtime_error("Tensor::get_slice_1d: The tensor shape size should be 2.");
            }
